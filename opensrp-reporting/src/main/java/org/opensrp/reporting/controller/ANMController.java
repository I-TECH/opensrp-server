...
public class ANMController {
	...
	public ANMController(@Value("#{opensrp['opensrp.site.url']}") String opensrpSiteUrl,
	    @Value("#{opensrp['opensrp.user.details.url']}") String opensrpUserDetailsUrl, ANMService anmService,
	    HttpAgent httpAgent, DetailsFetcherFactory detailsFetcherFactory) {
	...
	public ResponseEntity<List<ANMDTO>> all(@RequestParam("anm-id") String anmIdentifier) {
		...
		try {
			...
			UserDetail userDetail = new Gson().fromJson(response.body(), new TypeToken<UserDetail>() {}.getType());
			ANMDetailsFetcher anmDetailsFetcher = detailsFetcherFactory.detailsFetcher(userDetail.getRoles());
	...
	private List<ANMDTO> convertToDTO(List<SP_ANM> anms) {
		return with(anms).convert(new Converter<SP_ANM, ANMDTO>() {
			...
			private LocationDTO convertToDTO(Location location) {
				return new LocationDTO(location.subCenter(), location.phcName(), location.taluka(), location.district(),
				        location.state());
			}
		});
	}
